...
export default function NoteCard({ note, onPinToggle, onArchiveToggle }) {
  ...
  const togglePin = async () => {
    try {
      const res = await axios.patch(`${aps}/api/notes/${note._id}/pin`);
  ...
  const toggleArchive = async () => {
    try {
      const res = await axios.patch(`${aps}/api/notes/${note._id}/archive`);
  ...
  const handleEdit = () => {
    // Initialize modal state from current card values
    setModalTitle(title);
  ...
  const handleClose = () => {
    setOpenModal(false); // discard modal changes
  };

  const handleSave = async () => {
    try {
      const res = await axios.put(`${aps}/api/notes/${note._id}`, {
        title: modalTitle,
        content: modalContent,
        color: modalColor,
      });
      // update card state only after saving
      ...
      >
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex gap-1">
            ...
            <button className="p-1.5 rounded hover:bg-black/5 text-gray-600" title="More" onClick={(e) => e.stopPropagation()}>
